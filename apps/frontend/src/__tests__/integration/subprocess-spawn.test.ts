...
vi.mock('../../main/claude-profile-manager', () => {
  let mockProfileManager: {
    ...
    getActiveProfile: () => { profileId: string; profileName: string };
  };

  return {
    getClaudeProfileManager: () => {
      if (!mockProfileManager) {
        mockProfileManager = {
          hasValidAuth: vi.fn(() => true),
          getActiveProfile: vi.fn(() => ({ profileId: 'default', profileName: 'Default' }))
        };
      }
...
// Skip this test suite as it requires complex integration setup with Python
// These tests should be moved to E2E test suite
describe.skip('Subprocess Spawn Integration', () => {
  beforeEach(async () => {
    cleanupTestDirs();
    setupTestDirs();
    vi.clearAllMocks();
  ...
  describe('AgentManager', () => {
    it('should spawn Python process for spec creation', async () => {
      ...
      // Verify spawn was called (don't check exact args due to platform differences)
      expect(spawn).toHaveBeenCalled();
      const [command, args, options] = (spawn as ReturnType<typeof vi.fn>).mock.calls[0];

      // Check command is a Python executable
    ...
    it('should spawn Python process for task execution', async () => {
      ...
      expect(spawn).toHaveBeenCalled();
      const [command, args, options] = (spawn as ReturnType<typeof vi.fn>).mock.lastCall;
    ...
    it('should accept parallel options without affecting spawn args', async () => {
      ...
      // Verify spawn was called
      expect(spawn).toHaveBeenCalled();
      const [command, args] = (spawn as ReturnType<typeof vi.fn>).mock.lastCall;
    ...
    it('should use configured Python path', async () => {
      ...
      // Verify spawn was called with custom Python path
      expect(spawn).toHaveBeenCalled();
      const [command] = (spawn as ReturnType<typeof vi.fn>).mock.lastCall;
      expect(command).toBe('/custom/python3');
    });
