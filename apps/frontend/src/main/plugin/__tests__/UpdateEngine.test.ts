...
describe('UpdateEngine', () => {
  ...
  describe('applyUpdates', () => {
    ...
    it('should fail if backup creation fails', async () => {
      ...
      // Create a readonly directory to prevent backup creation
      const readonlyDir = path.join(TEST_PLUGIN_DIR, 'readonly');
      mkdirSync(readonlyDir, { recursive: true });

      // Override plugin path to point to a file (not a directory) to cause error
      // when trying to create backup directory inside it
      const testFile = path.join(TEST_PLUGIN_DIR, 'test-file.txt');
      writeFileSync(testFile, 'content');
      plugin.path = testFile;
